 ...
 *
 * The payload carries a path rather than a full URL on purpose — it is derived
 * from the picture's content and knows nothing about which host serves it. But
 * the app is served from a different origin than the API, so a bare `/public/…`
 * in an `<img src>` would resolve against the frontend and 404. The client is
 * the side that knows where the API lives.
 */
